...
const atualizaSituacao = async (
  candidatosRecebidos: Candidato[],
  tipo: "TI" | "COMERCIAL",
  msIntervalo = 300
) => {
  const candidatos = candidatosRecebidos.filter(
    (c) =>
      c.situacao.toLowerCase() != "empossado" &&
      c.situacao.toLowerCase() != "desistente"
  );
  ...
  return new Promise<void>(async (resolve) => {
    ...
    const intervalo: any = setInterval(async () => {
      ...
      let candidatoResposta = resposta.candidatos.find(
        (candidatoAntigo) => candidatoAntigo.nome == candidato.nome
      );
      if (!candidatoResposta) {
        console.log(
          "Candidato não encontrado na listagem, incluindo:",
          candidato.nome
        );
      ...
      try {
        ...
        const formulario = await capturaFormulario(
          candidato,
          resposta.data,
          axiosConfig
        );
        if (formulario)
          alteraSituacaoCandidato(candidatoResposta, formulario, tipo);
...
const atualizaRespostas = (tipo: "TI" | "COMERCIAL") => {
  let resposta: StatusCompleto =
    tipo == "COMERCIAL" ? RESPOSTA_COMERCIAL : RESPOSTA_TI;
  let autorizadas = 0;
  let cancelados = 0;
  let desistentes = 0;
  ...
  resposta.candidatos.forEach((candidato, indice) => {
    ...
    const candidatoOriginal = candidatosOriginais.find(
      (c) => c.nome == candidato.nome
    );
  ...
  if (candidatosNaoClassificados.length) {
    console.log(
      `Erro=> Candidatos não classificados:`,
      candidatosNaoClassificados
    );
